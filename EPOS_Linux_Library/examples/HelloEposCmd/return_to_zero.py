...
with live plotting of target vs. actual velocity.
"""
...
# Test profile
TARGET_RPM = 2000         # commanded speed
ACC_RPM_S = 1000
DEC_RPM_S = 1000
RUN_TIME = 10.0          # seconds at target before going back to zero
MONITOR_TIME = 10.0     # total monitoring time
...
if __name__ == "__main__":
    ...
    print(f"⚡ Test run at {TARGET_RPM} rpm for {RUN_TIME}s, then back to 0")
    ...
    try:
        while time.time() - t0 < MONITOR_TIME:
            ...
            # Switch to zero after RUN_TIME
            if elapsed > RUN_TIME and commanded_vel != 0:
                commanded_vel = 0
                move_with_velocity(h, 0)
                print("⬇️ Returning to zero velocity")
            ...
            # store values
            times.append(sample)
            target_vel.append(commanded_vel)
            ...
            if actual_vel:
                vmin = min(min(actual_vel), min(target_vel)) - 10
                vmax = max(max(actual_vel), max(target_vel)) + 10
                ax.set_ylim(vmin, vmax)

            fig.canvas.draw()
            fig.canvas.flush_events()

            print(f"📊 Target={commanded_vel} rpm, Actual={v} rpm, fault=0x{f:08X}", flush=True)
